      ...
      <SlideInCard direction="up" delay={0.2}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { href: "/issues", icon: AlertTriangle, color: "red", title: "Issues", desc: "Report problems" },
            { href: "/spaces", icon: Shield, color: "purple", title: "Spaces", desc: "Manage spaces" },
          ].map((item, i) => (
            <Link 
              key={i}
              href={item.href} 
            ...
            <div className="space-y-4">
              ...
              <Card className="border border-border/50 shadow-sm bg-white dark:bg-slate-900 rounded-4xl overflow-hidden">
                <CardContent className="p-2">
                  <button 
                    onClick={async () => {
                      try {
                        ...
                        if (err.message?.includes('already linked') || err.message?.includes('Identity is already linked')) {
                          toast.info('Google account is already linked');
                        } else {
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="h-10 w-10 rounded-lg bg-blue-500/5 flex items-center justify-center group-hover:scale-105 transition-transform">
                      ...
                      <div className="text-left">
                        <span className="font-bold block text-sm">Link Google Account</span>
                        <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">
